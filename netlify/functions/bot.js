...
async function enviarDocumentoTelegram(chatId, nome, conteudo) {
  const TELEGRAM_TOKEN = '123456789:ABCdefGhIJKlmNoPQRstUvWxYz1234567'; // ← SEU TOKEN PERMANECE AQUI
...
function gerarRelatorioTxt(processos, oabLabel, advogado) {
  let txt = `RELATÓRIO DE PROCESSOS - OAB ${oabLabel}\n`;
  txt += `Advogado: ${advogado || "Não informado"}\n`;
...
• Use \`/oab UF NÚMERO\` (ex: \`/oab MS 3616\`) para buscar por OAB`);
  ...
  // Busca por OAB — CHAMA NOSSA API PRÓPRIA
  if (texto.toLowerCase().startsWith('/oab')) {
    ...
    await enviarMensagemTelegram(chatId, `✅ Encontrados **${total} processos** para OAB ${labelOab}`);
  ...
  if (ehCpf) {
    ...
    if (processos.length) {
      await enviarMensagemTelegram(chatId, `✅ Encontrados **${processos.length} processos**`);
  ...
  await enviarMensagemTelegram(chatId, `⏳ Buscando por ${tipoBusca}...`);
