// Bot Telegram - versao simples para teste
...
// Faz requisicao HTTP GET ou POST
function doReq(host, path, method, headers, body) {
  return new Promise(function(ok, fail) {
    var r = https.request({
      hostname: host, path: path, method: method, headers: headers || {}
    }, function(res) {
      var d = '';
      ...
      for (var j = 0; j < at.length; j++) {
        ...
        if (at[j].advogados) {
          for (var k = 0; k < at[j].advogados.length; k++) {
            m += '  ADVOGADO: ' + at[j].advogados[k].nome;
            if (at[j].advogados[k].cpf) m += ' (CPF: ' + at[j].advogados[k].cpf + ')';
            m += '\n';
          }
        }
      }
...
exports.handler = function(event, context, callback) {
  // Responde GET com status (para teste)
  if (event.httpMethod === 'GET') {
    callback(null, { statusCode: 200, body: 'Bot ativo! Mande mensagem no Telegram.' });
    return;
  }

  if (event.httpMethod !== 'POST') {
    callback(null, { statusCode: 405, body: 'No' });
    return;
  }
  ...
  if (txt === '/start' || txt === '/help') {
    sendTg(chatId, 'Envie um NOME ou CPF/CNPJ para buscar processos.').then(function() {
      callback(null, { statusCode: 200, body: 'OK' });
    });
    return;
  }
  ...
  sendTg(chatId, 'Buscando...').then(function() {
    ...
    if (!dados || !dados.items || dados.items.length === 0) {
      return sendTg(chatId, 'Nenhum processo encontrado para: ' + txt);
    }
    if (dados.envolvido_encontrado) {
      sendTg(chatId, 'Encontrado: ' + dados.envolvido_encontrado.nome + ' (' + dados.envolvido_encontrado.quantidade_processos + ' processos)');
    }
    var promises = [];
    var max = Math.min(dados.items.length, 5);
    for (var i = 0; i < max; i++) {
      promises.push(sendTg(chatId, fmt(dados.items[i])));
    }
    return Promise.all(promises);
  }).then(function() {
    callback(null, { statusCode: 200, body: 'OK' });
  }).catch(function(e) {
    sendTg(chatId, 'Erro: ' + (e.message || e)).then(function() {
      callback(null, { statusCode: 200, body: 'OK' });
    }).catch(function() {
      callback(null, { statusCode: 200, body: 'OK' });
    });
