...
exports.handler = async (event) => {
  try {
    ...
    } catch (e) { console.log('CNJ indisponível:', e.message); }

    try {
      await new Promise(r => setTimeout(r, ATRASO_ENTRE_BUSCAS));
      const resOab = await buscarOAB(tipo, valor);
      processos.push(...resOab);
    } catch (e) { console.log('OAB indisponível:', e.message); }

    processos = processos.filter(p => p && p.numero_cnj).slice(0, LIMITE_RESULTADOS);

    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ total: processos.length, itens: processos, origem: "Nossa API - Fontes Oficiais" })
    };
  } catch (erro) {
...
async function buscarCNJ(tipo, valor) { return []; }
async function buscarOAB(tipo, valor) { return []; }
